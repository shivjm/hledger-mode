...
(defun hledger-completion-at-point ()
  ...
  (let ((pos (point)))
    (cond
     ((looking-back "\\(?:^\\|  \\);[[:space:]]*\\(?:[[:space:]]*[^[:space:]:]+:[^,]*,[[:space:]]*\\)*\\([^[:space:]:]*\\)\\(:\\([^,:]*\\)\\)?" (point-at-bol))
      (let* ((match (match-string-no-properties 1))
             (query (if (not (string-empty-p match))
                        (regexp-quote match)
                      nil))
             (is-value (not (null (match-string 2)))))
        (if is-value
            (list (match-beginning 3)
                  (match-end 3)
                  (hledger-get-tag-values query (current-buffer))
                  :exclusive 'no)
          (list (match-beginning 1)
                (match-end 1)
                (mapcar #'(lambda (tag) (concat tag ":")) (hledger-get-tags query (current-buffer)))
                :exclusive 'no))))
